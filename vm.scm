...
(define (grub!)
  (let ((program-counter (regvalue (register 'pc))))
	...
	(let ((grubbed (vector-ref program program-counter)))
...
(define (vm:step)
  (let ((instruction (grub!)))
    (case instruction
      
      ((load)
       (let* ((dest (register (grub!)))
	      (value (grub!)))
	 (set-register! dest value)))

      ((copy)
       (let* ((source (register (grub!)))
	      (dest (register (grub!))))
	 (set-register! dest (regvalue source))))

      ((push)
       (let ((src (register (grub!))))
	 (push-data! (regvalue src))))

      ((pop)
       (let ((dst (register (grub!))))
	 (set-register! dst (pop-data!))))
      ...
      ((jump-if-zero)
       (let* ((reg (register (grub!)))
	      (address (grub!)))
	 (if (zero? (regvalue reg))
	     (jump address))))

      ((jump-if-bigger)
       (let* ((reg1 (register (grub!)))
	      (reg2 (register (grub!)))
	      (addr (grub!)))
	 (if (> (regvalue reg1)
		(regvalue reg2))
	 ...
	 (set-register! reg (- (regvalue reg)))))
      ...
      ((out)
       (print (regvalue (register (grub!)))))

      ((in)
       (set-register! (register (grub!))
		      (read)))
